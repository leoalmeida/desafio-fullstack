...
describe('BeneficioService', () => {
  ...
  it('deve chamar endpoint ativar quando ativo for true', () => {
    ...
    const req = httpMock.expectOne(`${environment.beneficiosApi}/${mockBeneficio.id}/ativar`);
    expect(req.request.method).toBe('PUT');
    req.flush(mockBeneficio);
  });
  ...
  it('deve chamar endpoint cancelar quando ativo for false', () => {
    ...
    const req = httpMock.expectOne(`${environment.beneficiosApi}/${inativo.id}/cancelar`);
    expect(req.request.method).toBe('PUT');
    req.flush(inativo);
  });
